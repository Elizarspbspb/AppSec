...
### Что такое список
`Список` – это упорядоченная изменяемая коллекция элементов. Он может содержать что угодно: числа, строки, булевы значения, другие списки и даже словари.
...
Хотя так делать можно, на практике лучше использовать списки с однотипными элементами – это делает код предсказуемым и менее подверженным ошибкам.

`Списки` – это универсальная структура для:
1. `Хранения данных`: список пользователей, уязвимостей, IP-адресов, журналов событий.
2. `Фильтрации`: убрать ненужные элементы, например, безопасные домены из общего списка.
3. `Перебора`: пройтись по каждому элементу и выполнить анализ.
4. `Обработки данных из файлов и API`: JSON, CSV, отчёты сканеров.
```python
...
```
3. Преобразование других типов в список:
```python
...
1. Хранение URL для проверки уязвимостей:
```python
urls = ["http://test.com", "http://demo.com"]
for url in urls:
     print("Сканируем:", url)
```
2. Фильтрация подозрительных строк:
```python
...
`Словарь` – это неупорядоченная коллекция пар `«ключ–значение»`. Каждый ключ связан с каким-то значением, и вы можете мгновенно получить это значение, зная ключ.
...
print(user["name"])  # Bob
```
Если ключа нет, будет ошибка `KeyError`. Чтобы избежать этого, используйте метод `.get()` – он безопасен:
```py
