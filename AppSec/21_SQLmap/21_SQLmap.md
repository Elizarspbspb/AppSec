...
Дальше начинается перечисление структур.
Получив рабочую SQL-инъекцию и узнав тип СУБД, sqlmap может:
...
Рассмотрим несколько примеров использования sqlmap.
...
[INFO] the back-end DBMS is MySQL и информацию о том, какой параметр уязвим
...
Явно указывает, какой параметр тестировать на SQLi, например -p "id,username"
...
После нахождения SQLi перечисляет доступные базы данных на сервере
...
Перечисляет таблицы в указанной базе (-D)
...
Показывает список колонок в указанной таблице (-D + -T)
...
Дампит (извлекает) данные из выбранной таблицы/колонок
...
Выбирает конкретную базу данных, например -D app_db
...
Выбирает таблицу в выбранной базе, например -T users
...
Автоматически отвечает «по умолчанию» на все вопросы sqlmap (не задаёт интерактивных вопросов)
